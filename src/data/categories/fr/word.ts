export const wordHubHero = {
  title: "Outils Word gratuits en ligne",
  subtitle:
    "Convertissez vos documents Word en PDF et inversement, gratuitement, sans inscription et directement dans votre navigateur.",
  paragraphs: [
    "FileoPDF propose des outils Word gratuits pensés pour un usage rapide, sans installation et sans inscription. Chaque outil traite vos fichiers localement, dans votre navigateur, plutôt que de les envoyer vers un serveur distant pour les transformer.",
    "Cette approche limite l'exposition de vos documents, en particulier pour les fichiers sensibles. Vous gardez le contrôle sur vos fichiers du début à la fin du traitement.",
  ],
};